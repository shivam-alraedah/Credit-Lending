...
[CI](https://github.com/YOUR_GITHUB_USER/mal.ai/actions/workflows/ci.yml)
...
**Architecture document (canonical spec):** `[ARCHITECTURE.md](ARCHITECTURE.md)` → `[docs/architecture.md](docs/architecture.md)`.
...
## CI, tests, static PDFs

- **CI**: GitHub Actions runs `pytest` on every push/PR (`.github/workflows/ci.yml`).
- **Tests**: `pytest` from the repo root (see `pytest.ini`).
- **PDFs (static, checked in):** `exports/pdf/architecture.pdf` (≤8 pages), `plan_30_60_90.pdf` (≤3), `tradeoffs_and_product_analysis.pdf` (≤5). Regenerate: `./scripts/export_pdfs.sh` (Docker `pandoc/latex:3.6` or local xelatex). **IFRS9 SQL schema:** `ifrs9/01_schema.sql`.
...
docs/
  ...
  architecture.md       # system architecture specification (see ../ARCHITECTURE.md)
